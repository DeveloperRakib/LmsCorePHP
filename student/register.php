...
if(isset($_POST['submitbtn'])){ 
    ...
    if(count($input_errors)==0){ 
        $email_check = mysqli_query($dbcon, "SELECT * FROM `std` WHERE email='$email';");
        
        if(mysqli_num_rows($email_check)==0){
            $username_check = mysqli_query($dbcon,"SELECT * FROM std WHERE username ='$username';");
            if(mysqli_num_rows($username_check)==0){
                if($password==$cpassword){  
                    $password = md5($password);
                    $query = "INSERT INTO `std`(`fname`, `lname`, `roll`, `reg`, `email`, `username`, `password`,`status`, `phone`) VALUES ('$fname','$lname','$roll','$reg','$email','$username','$password','0','$number')";
